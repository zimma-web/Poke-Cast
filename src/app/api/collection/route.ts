...
export async function GET(request: Request) {
  try {
    ...
    // 4b. Find the user's top 10 rarest pulls
    const cardMap = new Map<string, any>(allCards.map((c: any) => [c.id, c]));
    const ownedCardIds = Object.keys(ownedCards);
    const rarestPulls = ownedCardIds
      .map(id => cardMap.get(id))
      .filter(Boolean)
      .sort((a, b) => getCardScore(b.rarity) - getCardScore(a.rarity))
      .slice(0, 10);
    ...
    const walletAddress = updatedUser?.wallet_address || null;
    let usdcBalance = 0;
    if (walletAddress) {
      try {
        const { getBaseUSDCBalance } = await import('@/lib/web3');
        usdcBalance = await getBaseUSDCBalance(walletAddress);
      } catch (e) {
        console.error('Failed to get Base USDC balance:', e);
      }
    }
